...
public class DrawFromExcel {  
	 ...
	 public static void test(String path) throws Exception {  
	        ...
	        int totalCol = excelList.get(excelList.size() - 1).size(); 
	        ...
	        for (Grid g : grids) {  
	            ...
	            g2d.setFont(font);  
	            System.out.println("----" + g.getText() + "," + g.getX() + "," + g.getY());
//	            g2d.drawString(g.getText(), g.getX() + (g.getWidth() - strWidth) / 2,  
//	                    g.getY() + (g.getHeight() - font.getSize()) / 2 + font.getSize());  
	            
	            g2d.drawString(g.getText(), g.getX() + (g.getWidth() - strWidth) ,  
	                    g.getY() + (g.getHeight() - font.getSize())  + font.getSize());  
	        }  
	        //表格最后一行有可能不显示，手动画上一行  
//	        g2d.drawLine(0, imageHeight - 100, imageWidth - 100, imageHeight - 100);  
	        g2d.dispose();  
